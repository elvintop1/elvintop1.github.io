window.quantumLessonNotes = {
  ...
  '10-parameterized-circuits': {
    explanation: [
      'A parameterized quantum circuit prepares a family of states $|\\psi(\\theta)\\rangle=U(\\theta)|0\\rangle$. The circuit architecture—the ansatz—determines which states can be reached, while a classical optimizer selects parameter values. More parameters increase flexibility but can also increase depth, sampling cost, and optimization difficulty.',
      'Problem-inspired ansätze preserve known symmetries or structure; hardware-efficient ansätze use native rotations and entanglers to reduce compilation overhead. The useful choice depends on the target state, hardware connectivity, noise level, and the measurements required by the objective.'
    ],
    keyIdeas: [
      'Expressibility is useful only when the reachable states include good solutions without making training impractical.',
      'Entangling pattern and two-qubit depth should match the backend connectivity.',
      'Parameter initialization can affect gradient scale, convergence, and reproducibility.'
    ],
    example: 'A two-layer ansatz may alternate $R_y(\\theta)$ rotations on every qubit with a linear chain of CNOT gates. Increasing the number of layers expands the reachable family but also adds noisy two-qubit operations.',
    pitfalls: ['Choosing an ansatz only because it contains many parameters.', 'Comparing ansätze after transpilation without reporting their resulting depth and gate counts.']
  },
  '10-expectation-value-objectives': {
    explanation: [
      'Variational objectives are often expectation values $C(\\theta)=\\langle\\psi(\\theta)|H|\\psi(\\theta)\\rangle$. When H is a weighted sum of Pauli strings, each term is estimated from measurement shots in a compatible basis and combined with its coefficient.',
      'Measurement grouping, shot allocation, and estimator uncertainty directly affect the noise seen by the classical optimizer. Commuting terms can sometimes share a measurement setting, but grouping rules and basis rotations must be specified.'
    ],
    keyIdeas: [
      'The cost estimate is stochastic at finite shot count.',
      'Allocate more shots to terms that contribute more variance or weight when justified.',
      'Transform observables with the same qubit layout used by the transpiled circuit.'
    ],
    example: 'For $H=0.7ZI-0.4XX+0.2IZ$, estimate ZI and IZ from Z-basis counts and XX after Hadamards on both qubits, then combine the three sample means with their coefficients.',
    pitfalls: ['Reporting only the optimizer’s final cost without an uncertainty estimate.', 'Measuring non-commuting Pauli terms in one setting without a valid joint-measurement method.']
  },
  '10-classical-optimization': {
    explanation: [
      'The classical optimizer proposes parameters, receives a noisy objective estimate, and repeats until a stopping rule is met. Gradient-free methods can tolerate some noise but may require many evaluations; gradient-based methods can use analytic parameter-shift estimates for suitable gates.',
      'Optimization failures may come from poor scaling, local minima, flat regions, shot noise, hardware drift, or an ansatz that cannot represent a good solution. Multiple initializations and learning curves help separate these causes.'
    ],
    keyIdeas: [
      'Count circuit evaluations, not only optimizer iterations.',
      'Use the same shot budget and stopping criteria when comparing optimizers.',
      'Parameter-shift gradients require additional circuit executions for each differentiated parameter.'
    ],
    example: 'For a rotation generated by a Pauli operator, a common derivative rule is $\\partial_\\theta C=[C(\\theta+\\pi/2)-C(\\theta-\\pi/2)]/2$. Estimating both shifted costs introduces sampling uncertainty into the gradient.',
    pitfalls: ['Calling the first converged point a global optimum.', 'Changing the optimizer, initialization, and shot budget simultaneously and attributing improvement to one factor.']
  },
  '10-vqe': {
    explanation: [
      'The variational quantum eigensolver minimizes a Hamiltonian expectation over an ansatz. In the ideal setting, the variational principle guarantees $E(\\theta)\\ge E_0$, so the best reachable trial state gives an upper bound to the true ground-state energy.',
      'A VQE workflow includes Hamiltonian construction, qubit mapping, symmetry or active-space reduction, ansatz preparation, grouped measurements, classical optimization, and validation against exact or trusted classical references for small instances.'
    ],
    keyIdeas: [
      'Ansatz error, measurement error, optimizer error, and modeling approximations are separate contributions.',
      'Energy accuracy does not by itself guarantee high state fidelity near degeneracies.',
      'Noise and mitigation can break the strict observed upper-bound behavior even though the ideal variational theorem remains valid.'
    ],
    example: 'For a two-qubit molecular Hamiltonian written as Pauli terms, prepare a symmetry-respecting trial state, estimate every grouped term, minimize the total energy, and compare with exact diagonalization of the same qubit Hamiltonian.',
    pitfalls: ['Comparing a noisy VQE result with a classical reference built from a different Hamiltonian or basis.', 'Reporting the lowest sampled energy without convergence history or repeated runs.']
  },
  '10-qaoa': {
    explanation: [
      'QAOA maps a classical objective to a diagonal cost Hamiltonian $H_C$ and alternates cost evolution with a mixer $H_M$. At depth p, parameters $\\gamma_k$ and $\\beta_k$ define a distribution over candidate bit strings rather than one deterministic answer.',
      'The measured objective expectation guides parameter optimization, while final samples are evaluated classically to identify feasible high-quality solutions. Encoding constraints, selecting a mixer, and defining approximation quality are problem-dependent choices.'
    ],
    keyIdeas: [
      'The cost Hamiltonian must assign energies consistently with the optimization objective.',
      'Increasing p expands the circuit family but raises depth and optimization cost.',
      'Expected cost and best sampled solution are different evaluation metrics.'
    ],
    example: 'For Max-Cut, each edge contributes $(I-Z_iZ_j)/2$. The standard mixer is $\\sum_iX_i$, and the initial state $|+\\rangle^{\\otimes n}$ represents all bit strings uniformly.',
    pitfalls: ['Declaring success from one favorable sampled bit string without its frequency.', 'Ignoring classical approximation algorithms when evaluating solution quality.']
  },
  '10-reliable-experiments': {
    explanation: [
      'A reliable variational experiment fixes the problem instance, metric, baseline, shot budget, optimizer settings, seeds, and stopping rules before comparison. It records the full cost trajectory, circuit evaluations, compiled resources, and final-solution distribution.',
      'Repeated runs expose sensitivity to initialization and stochastic noise. Ablations change one component at a time—ansatz, optimizer, mitigation, or backend—so the cause of a result can be identified.'
    ],
    keyIdeas: [
      'Use competitive exact, heuristic, or tensor-network baselines where scale permits.',
      'Report central tendency and variability across seeds or hardware runs.',
      'Preserve raw counts and calibration context for reproducibility.'
    ],
    example: 'Compare two ansätze using identical initial points, optimizer evaluation limits, and shot budgets across ten seeds. Report energy error, success probability, two-qubit depth, and total circuit executions.',
    pitfalls: ['Selecting only the best random seed after seeing all outcomes.', 'Calling a lower noisy objective better without checking uncertainty and solution validity.']
  },

  '11-where-qml-fits': {
    explanation: [
      'Quantum machine learning includes several settings: classical data processed by quantum models, quantum data analyzed by classical or quantum methods, and hybrid workflows. The potential advantage depends on data access, encoding cost, model class, training cost, and the strength of classical alternatives.',
      'A large Hilbert space is not evidence of useful learning by itself. The quantum feature map must preserve task-relevant structure, resist efficient classical simulation in the relevant regime, and lead to an end-to-end benefit under realistic resource accounting.'
    ],
    keyIdeas: [
      'State whether the input data are classical or natively quantum.',
      'Include data loading and repeated model evaluation in cost estimates.',
      'Separate research promise, empirical performance, and demonstrated quantum advantage.'
    ],
    example: 'A classifier for classically stored tabular data must include normalization, encoding, circuit execution, kernel or model evaluation, and classical training time—not only the duration of one quantum circuit.',
    pitfalls: ['Using “quantum neural network” as proof that a model inherits classical deep-learning benefits.', 'Comparing a tuned quantum model with an untuned or inappropriate classical baseline.']
  },
  '11-data-encoding': {
    explanation: [
      'Data encoding maps a classical vector x into a state $|\\phi(x)\\rangle=U(x)|0\\rangle$. Basis encoding uses computational states, angle and phase encoding use gate parameters, and amplitude encoding stores normalized features in amplitudes. Each method trades qubit count, depth, normalization, and state-preparation complexity.',
      'Amplitude encoding can represent N features with $\\lceil\\log_2N\\rceil$ qubits, but preparing an arbitrary amplitude vector may be expensive. Angle encoding is often shallow but commonly uses roughly one qubit per feature unless data re-uploading is used.'
    ],
    keyIdeas: [
      'Classical preprocessing and quantum-state normalization are different operations.',
      'Periodic rotation encodings require a scaling convention to avoid unintended aliasing.',
      'The best encoding is task- and hardware-dependent.'
    ],
    example: 'Angle encoding can map feature $x_k$ to $R_y(x_k)|0\\rangle$. Amplitude encoding maps a normalized vector to $\\sum_i x_i|i\\rangle$, but needs a state-preparation circuit whose cost must be counted.',
    pitfalls: ['Claiming exponential storage benefit while treating arbitrary amplitude loading as free.', 'Using test-set information when fitting classical normalization parameters.']
  },
  '11-feature-maps': {
    explanation: [
      'A quantum feature map uses a data-dependent unitary to embed inputs in a quantum state space. Rotation layers, data-dependent phases, entanglers, and repeated data uploads determine which relationships between inputs become accessible to later measurements.',
      'Feature-map design should consider inductive bias, circuit depth, hardware connectivity, noise, and classical simulability. More entanglement or more repetitions do not automatically improve generalization.'
    ],
    keyIdeas: [
      'Keep feature order and scaling consistent between training and inference.',
      'Data re-uploading can increase functional expressivity without increasing qubit count.',
      'A useful feature map must be evaluated on held-out data, not only by state-space dimension.'
    ],
    example: 'A ZZ feature map can alternate Hadamards with data-dependent Z and ZZ phase rotations. The ZZ terms encode pairwise feature interactions but also add two-qubit depth after compilation.',
    pitfalls: ['Selecting a feature map only because it is believed to be hard to simulate.', 'Increasing repetitions until training accuracy rises without monitoring validation performance and hardware cost.']
  },
  '11-quantum-kernels': {
    explanation: [
      'A quantum kernel estimates similarity after encoding, commonly $K(x_i,x_j)=|\\langle\\phi(x_i)|\\phi(x_j)\\rangle|^2$. The resulting kernel matrix can be supplied to a classical support-vector machine or other kernel method.',
      'Building an M-by-M kernel matrix requires many pairwise circuit estimates. Finite shots can make the estimated matrix noisy or slightly non-positive, so circuit cost, symmetry, regularization, and matrix conditioning should be reported.'
    ],
    keyIdeas: [
      'The feature map defines the kernel and its inductive bias.',
      'Use identical preprocessing and circuit compilation across data splits.',
      'Compare with linear, polynomial, and RBF kernels using matched model-selection effort.'
    ],
    example: 'Prepare $U(x_i)|0\\rangle$, apply $U^\\dagger(x_j)$, and estimate the all-zero probability. Under the standard overlap circuit, that probability equals the fidelity kernel entry.',
    pitfalls: ['Computing the test–test kernel and accidentally leaking it into training.', 'Claiming advantage from higher feature-space dimension without a scaling or classical-hardness argument.']
  },
  '11-variational-classifiers': {
    explanation: [
      'A variational quantum classifier combines a feature map $U(x)$, a trainable circuit $V(\\theta)$, and a measured observable or sampled bit-string distribution. A classical loss compares predictions with labels and updates the trainable parameters.',
      'Estimator-based models return expectation values; sampler-based models return outcome probabilities. The interpretation rule, loss, threshold, class weighting, and gradient method are part of the model and must be fixed using training or validation data only.'
    ],
    keyIdeas: [
      'Feature-map parameters encode data; ansatz parameters are learned.',
      'Training requires repeated circuit executions across samples, parameters, and possibly gradient shifts.',
      'Accuracy alone may hide class imbalance; include appropriate precision, recall, F1, or calibration metrics.'
    ],
    example: 'Encode two features with $R_y$ rotations, apply a trainable entangling layer, measure Z on one qubit, and map the expectation sign to a binary label. Optimize cross-entropy or another stated loss on the training set.',
    pitfalls: ['Reporting training accuracy as model performance.', 'Calling every parameterized circuit a neural network without explaining the model output and training rule.']
  },
  '11-evidence-standards': {
    explanation: [
      'A credible QML study separates training, validation, and untouched test data; fits preprocessing only on the training split; and gives quantum and classical models comparable tuning effort. It reports seeds, data splits, circuit resources, shot counts, runtime, uncertainty, and failed runs.',
      'Evidence for advantage requires more than better accuracy on one small dataset. Scaling behavior, strong baselines, total resource cost, noise sensitivity, and reproducibility must support the claim.'
    ],
    keyIdeas: [
      'Use cross-validation or repeated splits when the dataset is small.',
      'Report confidence intervals or variation across seeds rather than one number.',
      'Ablate encoding, entanglement, depth, and mitigation to identify what contributes to performance.'
    ],
    example: 'Evaluate quantum kernel, RBF-SVM, linear-SVM, and a tree-based baseline on identical nested cross-validation splits. Record test performance only after hyperparameters are selected without test access.',
    pitfalls: ['Using the test set repeatedly during feature-map or hyperparameter design.', 'Calling a statistically indistinguishable score improvement a quantum advantage.']
  }
};
